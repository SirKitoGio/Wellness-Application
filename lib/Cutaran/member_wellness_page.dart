...
extension DayOfWeekExtension on DayOfWeek {
  ...
  String get displayName => nameLower[0].toUpperCase() + nameLower.substring(1);
}
...
class _MemberWellnessPageState extends State<MemberWellnessPage> {
  ...
  Future<void> getWellnessPlans() async {
    ...
    try {
      final uri = Uri.parse(
        '$_apiBaseUrl/get/wellness/plan/${widget.groupId}/${widget.memberId}',
      );
      final response = await http.get(uri);
      if (response.statusCode == 200) {
        final decoded = jsonDecode(response.body);
        final plans = (decoded is Map && decoded.containsKey('body'))
            ? decoded['body']
            : decoded;
        setState(() {
          wellnessPlans = (plans is List) ? plans : [];
        ...
        _showSnackBar(
          'Failed to load plans: ${response.statusCode} ${response.reasonPhrase}',
        );
      }
    } catch (e) {
      setState(() {
  ...
  Future<void> createWellnessPlan() async {
    ...
    final body = {
      ...
      'dayOfWeek': selectedDay!.nameLower,
      'diet_plan': dietCtrlr.text.trim(),
      'work_plan': workoutCtrlr.text.trim(),
      'tips': tipsCtrlr.text.trim(),
    };
    try {
      final uri = Uri.parse('$_apiBaseUrl/create/wellness/plan');
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        _showSnackBar('Wellness plan created.');
  ...
  Widget _buildExistingPlansSection() {
    ...
    if (wellnessPlans.isEmpty)
      return const Padding(
        ...
        child: Text('No existing plans.'),
      );
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: wellnessPlans.map((plan) {
        final day =
            plan['day_of_week'] ?? plan['day'] ?? plan['dayofWeek'] ?? 'N/A';
        final diet = plan['diet_plan'] ?? plan['diet'] ?? 'N/A';
        final workout = plan['work_plan'] ?? plan['workout'] ?? 'N/A';
